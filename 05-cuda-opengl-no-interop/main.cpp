...
void cudaRenderImageToDeviceBuffer(uchar3 *&d_image) {
...
int main() {
  ...
  uchar3 *cudaDeviceRenderBuffer;
  cudaMalloc((void **) &cudaDeviceRenderBuffer, sizeof(uchar3) * WIDTH * HEIGHT);
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    // Render cuda image
    cudaRenderImageToDeviceBuffer(cudaDeviceRenderBuffer);

    // Copy to host
    cudaMemcpy(
        hostImage.data(),
        cudaDeviceRenderBuffer,
        sizeof(uchar3) * WIDTH * HEIGHT,
        cudaMemcpyDeviceToHost
    );

    // Update texture
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, WIDTH, HEIGHT, 0, GL_RGB, GL_UNSIGNED_BYTE, hostImage.data());
